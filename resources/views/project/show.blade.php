...
<div class="container-fluid p-0 pb-5 pb-md-0">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        ...
                        @forelse($project->suratJalans as $sj)
                            <li><a class="dropdown-item" href="{{ route('surat-jalan.show', $sj) }}"><i class="bi bi-eye me-1"></i> Lihat {{ $sj->nomor }}</a></li>
                            @if(auth()->user()->hasPermission('surat_jalan', 'print'))
                                <li><a class="dropdown-item" href="{{ route('surat-jalan.download', $sj) }}"><i class="bi bi-printer me-1"></i> Cetak {{ $sj->nomor }}</a></li>
                            @endif
                        @empty
                            <li><span class="dropdown-item-text text-muted small">Belum ada Surat Jalan</span></li>
                        @endforelse
                    </ul>
                </div>
            @endif
    ...
    <div class="row g-3 mb-3">
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-3 h-100">
                <div class="card-body">
                    <h6 class="fw-bold mb-3">Informasi Kunci</h6>
            ...
            <div class="card border-0 shadow-sm rounded-3">
                <div class="card-body">
                    ...
                    <div class="d-flex flex-column gap-2">
                        @if(auth()->user()->hasPermission('surat_jalan', 'create'))
                            <a href="{{ route('surat-jalan.create', $project) }}" class="btn btn-sm btn-outline-primary text-start">
                                <i class="bi bi-file-earmark-plus me-1"></i> Buat Surat Jalan
                            </a>
                        @endif
                        <a href="#tab-barang" data-bs-toggle="tab" class="btn btn-sm btn-outline-secondary text-start">
                            <i class="bi bi-box-seam me-1"></i> Daftar Barang Project
                        </a>
                        <a href="#tab-dokumen" data-bs-toggle="tab" class="btn btn-sm btn-outline-secondary text-start">
    ...
    <ul class="nav nav-tabs mb-3 d-none d-md-flex" id="projectTabs" role="tablist">
        ...
        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-kanban" type="button">Kanban</button></li>
        ...
        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-catatan" type="button">Catatan</button></li>
    </ul>
    ...
    <div class="tab-content">
        <div class="tab-pane fade show active" id="tab-overview">
            <div class="card border-0 shadow-sm rounded-3">
                <div class="card-body">
                    <h6 class="fw-bold mb-3">Ringkasan Project</h6>
                    <p class="small text-muted">Dibuat oleh {{ $project->creator->name ?? '-' }} pada {{ $project->created_at->translatedFormat('d M Y, H:i') }} WIB.</p>
                    <div class="row g-3 small">
                        <div class="col-md-4">
                            <div class="text-muted">Kategori</div>
                            <div class="fw-semibold">{{ $project->category }}</div>
                        </div>
                        <div class="col-md-4">
                            <div class="text-muted">Jumlah Crew</div>
                            <div class="fw-semibold">{{ $project->crews->count() }} orang</div>
                        </div>
                        <div class="col-md-4">
                            <div class="text-muted">Jumlah Surat Jalan</div>
                            <div class="fw-semibold">{{ $project->suratJalans->count() }}</div>
                        </div>
                    </div>
                </div>
            </div>
...
<nav class="d-md-none fixed-bottom bg-white border-top shadow-sm">
    <div class="d-flex justify-content-around py-2">
        <a href="#tab-overview" data-bs-toggle="tab" class="text-center text-decoration-none text-secondary small mobile-tab-link active">
            <i class="bi bi-house d-block fs-5"></i>Overview
        </a>
        <a href="#tab-kanban" data-bs-toggle="tab" class="text-center text-decoration-none text-secondary small mobile-tab-link">
