...
<body>
    ...
    <div class="content-body">
        ...
        <div class="media-section">
            <!-- Foto Aset Barang -->
            <div class="box-image">
                @if($inventory->image && file_exists(public_path('storage/' . $inventory->image)))
                    <img src="{{ public_path('storage/' . $inventory->image) }}" class="inventory-photo">
                @else
                    <div class="no-photo-placeholder">TIDAK ADA FOTO ASSET BARANG</div>
                @endif
            </div>

            <!-- Identitas QR Code Unik -->
            <div class="box-qrcode">
                @if($inventory->qr_code && file_exists(public_path('storage/' . $inventory->qr_code)))
                    <img src="{{ public_path('storage/' . $inventory->qr_code) }}" class="qr-code-img">
                    <div class="qr-caption">{{ $inventory->serial_number }}</div>
                @else
                    <div style="color: red; font-size: 10pt; font-weight: bold; padding: 40px 0;">QR CODE FISIK TIDAK DITEMUKAN</div>
                @endif
            </div>
